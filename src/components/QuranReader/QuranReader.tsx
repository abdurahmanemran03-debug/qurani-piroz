...
// Static Kurdish reciters supplied for direct audio playback.
// Their exact audio servers are taken from the supplied base URLs.
const STATIC_KURDISH_RECITERS: DynamicReciter[] = [
  {
    id: 'raad_kurdish',
    sourceId: 'raad',
    name: 'ڕەعد کوردی',
    nameAr: 'رعد الكردي',
    riwayah: 'حفص عن عاصم',
    server: 'https://download.way2quran.com/files/kurdish/raad/',
  ...
  },
  {
    ...
    nameAr: 'رزكار محمد الكردي',
    riwayah: 'حفص عن عاصم',
    server: 'https://download.way2quran.com/files/kurdish/rzgar/',
    surahList: Array.from({ length: 114 }, (_, i) => i + 1),
    surahTotal: 114,
    moshafId: 'rzgar',
    source: 'mp3quran',
  },
  {
    ...
    nameAr: 'فرمان شواني',
    riwayah: 'حفص عن عاصم',
    server: 'https://download.way2quran.com/files/kurdish/firman/',
    surahList: Array.from({ length: 114 }, (_, i) => i + 1),
    surahTotal: 114,
    moshafId: 'firman',
    source: 'mp3quran',
  },
  {
    ...
    nameAr: 'پێشەوا قادر الكردي',
    riwayah: 'حفص عن عاصم',
    server: 'https://download.way2quran.com/files/kurdish/peshawa/',
    surahList: Array.from({ length: 114 }, (_, i) => i + 1),
    surahTotal: 114,
    moshafId: 'peshawa',
    source: 'mp3quran',
...
> {
  ...
  // First keep API-discovered entries.
  ...
  // Add the supplied direct-server reciters when the API does not return them.
  for (const item of STATIC_KURDISH_RECITERS) {
  ...
  const ordered = [
    ...KURDISH_RECITER_ALIASES.map((alias) =>
      unique.get(alias.id),
    ).filter(
      (item): item is DynamicReciter => Boolean(item),
    ),
    ...STATIC_KURDISH_RECITERS.filter(
      (item) => !KURDISH_RECITER_ALIASES.some((alias) => alias.id === item.id),
    ),
  ];
...
export function QuranReader({
  ...
  const [ayahCoordinates, setAyahCoordinates] =
  ...
  /*
   * Load the real polygon coordinates for every ayah on the current page.
   * The coordinate source uses the same 345x550 Mushaf page coordinate
   * system, so the SVG overlay lines up with the page image.
   */
  useEffect(() => {
    let cancelled = false;
    ...
    const load = async () => {
      try {
        const coordinates =
          await getAyahBoxesForPage(currentPage);
  ...
  const renderAyahAreas =
    useCallback(() => {
      if (!ayahs.length || !ayahCoordinates.length) {
        return null;
      }

      /*
       * IMPORTANT:
       * Do NOT divide the page into equal horizontal rows.
       * Every ayah is drawn from its real polygon coordinates.
       *
       * The source coordinate system is 345 x 550. SVG's default
       * xMidYMid meet behaviour matches the image's objectFit:
       * contain, so the polygons stay aligned with the Mushaf.
       */
      return (
        <div
          aria-label="Ayah interaction layer"
          style={{
            position: 'absolute',
            inset: 0,
            zIndex: 20,
        ...
        >
          ...
          >
            {ayahCoordinates.map((coordinate) => {
              const index = ayahs.findIndex((ayah) => {
                const ayahSurah = Number(
                  ayah?.surahNumber ??
                    (ayah?.surah as any)?.number ??
                    selectedSurahNumber,
                );

                const ayahNumber = Number(
                  ayah?.ayah ??
                    (ayah as any)?.numberInSurah ??
                    0,
                );

                return (
                  ayahSurah ===
                    Number(coordinate.surahNumber) &&
                  ayahNumber ===
                    Number(coordinate.ayahNumber)
                );
              });
              ...
              return (
                <polygon
                  key={`${currentPage}-${coordinate.surahNumber}-${coordinate.ayahNumber}`}
